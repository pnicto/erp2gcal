...
# Course class
class Course:
    def __init__(self, name, class_type, room, days, start, end):
        self.name = name
        self.class_type = class_type  # LAB,LEC,TUT
        self.days = days
        self.start = start
        self.room = room
        self.end = end
...
class CourseParser:
    # Function which creates corresponsing time intervals depending on the class type of course
    # Workshop Lab is 3hrs but this still takes it as 2 hours
    # Fixed it with a WSFLAG :p
    def start_end__time_generator(inpt, class_type, WSFLAG=False):
    ...
    def course_instance_generator(erp_registered_courses):
        courses = []
        for i in range(0, len(erp_registered_courses), 4):
            ...
            start, end = CourseParser.start_end__time_generator(
